...
the operator's cpu list minus weka's pinned cores and minus any cpu the host
refuses to bind (every cpu minus weka's when no list is given), so it can
never go stale when weka is re-pinned.
