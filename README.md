...
**Partial layouts heal themselves.** fio sizes a file *before* writing its data, so an interrupted layout (Ctrl-C, out of space) leaves full-size files that are mostly holes — and fio's own size check would trust them forever, quietly benchmarking zero-filled reads. wekatester therefore keeps a completion marker (`.wekatester-layout-<hash>.done` under `-d`, one per layout geometry) that is removed before every layout run and written back only after the layout verifiably completed. When the marker is missing — first run, interrupted layout, or changed geometry — the layout switches to full writes and rebuilds every file end to end, saying so in the log. Expect exactly that (a longer, louder layout phase) on the first run after any interrupted one; healthy repeat runs are as fast as ever. To force a rebuild by hand: `rm '<dir>'/.wekatester-layout-*.done`.
