#!/usr/bin/python2.7
""" 
    Utility for converting *_clat_hist* files generated by fio.last into latency statistics.
...
def _plat_idx_to_val(idx, edge=0.5, FIO_IO_U_PLAT_BITS=6, FIO_IO_U_PLAT_VAL=64):
    """ Taken from fio.last's stat.c for calculating the latency value of a bin
...
def plat_idx_to_val_coarse(idx, coarseness, edge=0.5):
    """ Converts the given *coarse* index into a non-coarse index as used by fio.last
...
def guess_max_from_bins(ctx, hist_cols):
    ...
    if len(idx[1]) == 0:
        ...
        errmsg = ("Unable to determine bin values from input clat_hist files. Namely \n"
            ...
            "  - You recompiled fio.last with a different GROUP_NR. If so please specify this\n"
...
if __name__ == '__main__':
    ...
    arg('--job-file',
        ...
        help='Optional argument pointing to the job file used to create the '
             ...
             '--log_unix_epoch (in fio.last) values.')
    ...
    arg('--usbin',
        default=False,
        action='store_true',
        help='histogram bin latencies are in us (fio.last versions < 2.99. fio.last uses ns for version >= 2.99')
